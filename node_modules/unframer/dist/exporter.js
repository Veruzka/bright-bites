...
exports.propCamelCase = propCamelCase;
...
async function bundle({ config, cwd: out = '', watch = false, signal = undefined, }) {
    ...
    async function rebuild() {
        ...
        const buildResult = await buildContext.rebuild().catch((e) => {
            if (e.message.includes('No matching export ')) {
                foundError = true;
                utils_js_1.spinner.error(`esbuild failed to import from an external package, this usually means that the npm package version in Framer is older than the latest.`);
                utils_js_1.spinner.error(`Use --external to make all npm packagess external, then install the right version`);
                process.exit(1);
            }
            throw e;
        });
        utils_js_1.spinner.update('Finished build');
        for (let file of buildResult.outputFiles) {
            ...
            await fs_1.default.promises.mkdir(path_1.default.dirname(resultPathAbs), {
                recursive: true,
            });
            await fs_1.default.promises.writeFile(resultPathAbs, codeNew, 'utf-8');
        }
        utils_js_1.spinner.stop();
        ...
        const sema = new async_sema_1.Sema(utils_js_1.stackblitzDemoExample ? 5 : 10);
        utils_js_1.spinner.update('Extracting types');
        utils_js_1.logger.log(`using node path`, nodePath);
        let allFonts = [];
        const propControlsData = await Promise.all(buildResult?.outputFiles.map(async (file) => {
            try {
                ...
                utils_js_1.spinner.update(`Extracting types for ${name}`);
        ...
        const cssString = doNotEditComment +
            ...
            (0, css_js_1.breakpointsStylesLegacy)(breakpoints) +
            '\n\n' +
            framer_js_1.combinedCSSRules
                .map((x) => (x?.startsWith('  ') ? (0, string_dedent_1.default)(x) : x))
                .join('\n') +
        ...
        const outFiles = buildResult.outputFiles
            ...
            .concat(buildResult.outputFiles.map((x) => path_1.default.resolve(out, x.path.replace('.js', '.d.ts'))));
        ...
        utils_js_1.spinner.info(`Build completed`);
        return res;
    }
    ...
    const responsiveComponent = (() => {
        return (0, string_dedent_1.default) `
    ...
    if (utils_js_1.stackblitzDemoExample) {
        ...
        await fs_1.default.promises.writeFile(utils_js_1.stackblitzDemoExample, exampleCode);
    }
    if (!foundError) {
        console.log((0, utils_js_1.terminalMarkdown)((0, string_dedent_1.default) `
...
function resolvePackage({ cwd, pkg }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.exec)(command, {
            ...
            if (error) {
                utils_js_1.logger.log(stderr);
                reject(new Error('Unframer is not installed in your project'));
                return;
            }
...
async function checkUnframerVersion({ cwd }) {
    ...
    try {
        ...
        if (installedVersion !== currentVersion) {
            utils_js_1.spinner.error(`IMPORTANT: Unframer version mismatch. Please run: npm update unframer@latest`);
        }
    }
    catch (e) {
        utils_js_1.spinner.error('IMPORTANT: Unframer is not installed in your project. Please run: npm install unframer');
    }
}
...
function getStyleTokensCss(tokens, darkModeType = 'class') {
    ...
    const lightUnframerTokens = tokens
        .map((token) => '    --unframer-' +
        (0, utils_js_1.kebabCase)(token.name || token.id) +
        ': ' +
        token.lightColor +
        ';')
        .join('\n');
...
async function extractPropControlsUnsafe(filename, name) {
    ...
    let stdout = await new Promise((res, rej) => {
        ...
        let childProcess = (0, child_process_1.exec)(cmd, {
            ...
            clearTimeout(timer);
            if (err) {
                utils_js_1.spinner.error(`error extracting types for ${name}`);
