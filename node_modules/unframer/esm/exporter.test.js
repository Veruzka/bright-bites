...
describe('propCamelCase', () => {
    test('converts dashes to camelCase', () => {
        ...
        expect(propCamelCase('foo-bar-baz')).toMatchInlineSnapshot(`"fooBarBaz"`);
    });
    test('converts example-variable_dashes etc XX something 11 to camelCase', () => {
        expect(propCamelCase('example-variable_dashes etc XX something 11')).toBe(`exampleVariableDashesEtcXXSomething11`);
    });
    test('converts 0 starts with ZeroEtc to camelCase', () => {
        expect(propCamelCase('0 starts with ZeroEtc')).toBe(`_0StartsWithZeroEtc`);
    });
    test('converts underscores to camelCase', () => {
        expect(propCamelCase('foo_bar')).toMatchInlineSnapshot(`"fooBar"`);
        expect(propCamelCase('foo_bar_baz')).toMatchInlineSnapshot(`"fooBarBaz"`);
    });
    ...
    test('handles mixed cases', () => {
        expect(propCamelCase('Foo-Bar_Baz Test')).toMatchInlineSnapshot(`"fooBar_BazTest"`);
    });
